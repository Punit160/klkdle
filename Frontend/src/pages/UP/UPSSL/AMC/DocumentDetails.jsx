...
const parseUniqueIds = (unique_id) => {
    if (Array.isArray(unique_id)) {
        return unique_id
    }

    if (typeof unique_id === 'string' && unique_id.trim()) {
        try {
            const parsed = JSON.parse(unique_id)
...
const SslUniqueIdCard = ({ sslIds, uniqueIds }) => {
    const rowCount = Math.max(sslIds.length, uniqueIds.length)
    const [isOpen, setIsOpen] = useState(false)

    if (rowCount === 0) {
        return (
            <div className="border rounded-3 p-3 text-center text-muted fs-12 bg-light">
                No SSL ID / Unique ID data for this document.
            </div>
        )
    }
    ...
    return (
        <div className="border rounded-3 overflow-hidden">
            ...
            >
                ...
                <span className="badge bg-soft-primary text-primary ms-auto rounded-pill">
                    {rowCount} Sites
                </span>
                ...
                >
                    {Array.from({ length: rowCount }).map((_, i) => (
                        ...
                        >
                            ...
                            {uniqueIds[i] !== undefined ? (
                                <span className="badge bg-soft-secondary text-secondary fs-10">{uniqueIds[i]}</span>
                            ) : (
...
const DocumentEntryCard = ({ entry, isEditing, onStartEdit, onCancelEdit, onSave, editRemarks, setEditRemarks, editAmcFile, setEditAmcFile, editInvoiceFile, setEditInvoiceFile, isSaving }) => {
    const amcUrl = entry.amc_document_url
    const invoiceUrl = entry.invoice_document_url
    const sslIds = parseSslIds(entry.ssl_id)
    const uniqueIds = parseUniqueIds(entry.unique_id)
    ...
    return (
        <div className="rounded-3 p-3 mb-2" style={{ border: '1px dashed #dde1ea', background: '#fbfcfe' }}>
            ...
            <SslUniqueIdCard sslIds={sslIds} uniqueIds={uniqueIds} />
        </div>
    )
}
...
const DocumentDetails = () => {
    ...
    const saveEdit = async (entry) => {
        ...
        try {
            ...
            formData.append('up_ssl_amc_id', entry.up_ssl_amc_id || row.id)
            ...
            <>
                ...
                <div className="main-content">
                    <div className="text-center py-5">
                        ...
                        <button type="button" className="btn btn-light" onClick={() => navigate('/uttarpradesh/ssl-amc/view-document')}>
                                    ...
                                    >
                                        ...
                                        <div className="d-flex align-items-center gap-2 mb-3">
                                            ...
                                            <span className="fs-11 text-muted flex-shrink-0">
                                                {completed}/{total} completed &middot; {pending} pending
                                            </span>
                                        </div>
