...
const SelectDropdown = ({ options, selectedOption, onSelectOption, className, defaultSelect }) => {
    ...
    useEffect(() => {
        if (defaultSelect) {
            ...
            if (defaultOption) {
                setLocalSelectedOption(defaultOption)
            }
        }
    ...
    useEffect(() => {
        ...
        const updated = options?.find((option) => option.value === localSelectedOption.value)
        if (updated && updated.label !== localSelectedOption.label) {
            setLocalSelectedOption(updated)
        }
    }, [options, localSelectedOption])
...
const filteredOptions = options?.filter(option =>
    ...
    useEffect(() => {
        if (isOpen) {
            ...
            if (dropdownRect.bottom + 360 > windowHeight) {
                setOpenUpwards(true);
            } else {
